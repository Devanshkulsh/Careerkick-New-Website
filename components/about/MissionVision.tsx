...
function MissionVisionComponent() {
    ...
    return MISSION_VISION_ITEMS.map((item, index) => {
      ...
      return (
        <motion.article
          ...
          className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.05] p-8 shadow-card backdrop-blur-xl transition-all duration-500 hover:-translate-y-1 hover:border-violet/40 hover:bg-white/[0.08] hover:shadow-elevated sm:p-10"
        >
          <div className="pointer-events-none absolute inset-0 rounded-3xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]" />

          <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-violet/25 bg-violet/10 text-violet-glow shadow-glow-violet transition-transform duration-500 group-hover:scale-110 group-hover:bg-violet/15">
            <Icon size={26} strokeWidth={1.5} />
          </div>
          <h3 className="mt-6 font-display text-2xl font-bold tracking-normal text-white">
            {item.title}
          </h3>
  ...
  return (
    <SectionWrapper
      id="mission-vision"
      className="relative overflow-hidden bg-base px-6 py-24 sm:px-8 lg:px-14"
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        {snowParticles.map((flake) => (
          <motion.div
            key={flake.id}
            className="absolute rounded-full bg-violet-glow blur-[1px]"
            style={{
              left: flake.left,
              width: flake.size,
              height: flake.size,
